    ...
    for i in range(n_samples):
        ...
        for j in range(len(x[i])):
            if x[i][j] != len(We) - 1:
                ...
                num = len(np.array(x[i][max(0, j - win) : min(len(x[i]), j + win + 1)]))
                #window size, used to rescale
                averg = (np.sum(We[np.array(x[i][max(0, j - win) : min(len(x[i]), j + win + 1)])], axis=0) - We[x[i][j]]) / num
                act = np.dot(averg, A)
                set.append(act)
...
def SIF_embedding_pc(We, x, w, win, A):
    """
